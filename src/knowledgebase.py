...
class QdrantKnowledgeBase:
    ...
    async def _add_texts(
        ...
        points = [
            qmodels.PointStruct(
                ...
                payload={"text": text, "rule_type": rule_type},
            )
    ...
    async def fill_base(
        ...
        await self._add_texts(
            rules, rule_embeddings, collection_name, rule_type, vector_size
        )
            ...
            filters = qmodels.Filter(
                must=[
                    qmodels.FieldCondition(
                        key="rule_type", match=qmodels.MatchValue(value=rule_type)
                    )
                ]
            )

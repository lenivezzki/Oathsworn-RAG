...
class ChunksReranker:
    ...
    async def rank(self, query: str, top_k:int=3):
        if self.bm25 is None or self.chunk_embs is None:
            raise ValueError("Ранкер должен быть подготовлен с помощью метода `fit` перед использованием.")
        ...
        ranked_chunks = [(text,score) for score,text in sorted(zip(final_scores, self.chunks), reverse=True)]
